...
def upload_file():
    ...
    if file:
        ...
        if filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):  # Video formats
            ...
            # Load image explicitly to ensure it exists and is valid
            frame = cv2.imread(file_path)
            if frame is None:
                print(f"Error: Could not load image from {file_path}")
                flash('Error loading uploaded image. Please try another file.', 'error')
                return redirect(url_for('about'))
...
def play_alarm():
    """Plays alarm sound without blocking detection loop."""
